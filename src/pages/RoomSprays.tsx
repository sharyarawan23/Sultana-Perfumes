...
const useCards = [
  {
    title: "HOME",
    subtitle: "A fresher, warmer space.",
    image: "/assets/use-home.png",
  },
  {
    title: "CAR",
    subtitle: "A luxurious drive.",
    image: "/assets/use-car.png",
  },
  {
    title: "CLOTHES & LINEN",
    subtitle: "Lasting freshness.",
    image: "/assets/use-clothes.png",
  },
  {
    title: "ROOM & SPA",
    subtitle: "A relaxing atmosphere.",
    image: "/assets/use-living.png",
  },
];
...
export default function RoomSprays() {
  ...
  const sprays = useMemo(
    () => products.filter((product) => product.category === "spray"),
    []
  );
    ...
    <main className="bg-[#fbf9f5] text-[#18140f]">
      <section className="w-full overflow-hidden">
        <picture>
          ...
          <img
            src="/assets/campaigns/hero-spray-unified.webp"
            alt="Luxury Home Spray"
      ...
      <section className="px-5 md:px-12 lg:px-20 py-14 md:py-20">
        <div className="flex items-end justify-between gap-6 mb-8">
          <div>
            <p className="text-[11px] tracking-[0.34em] uppercase text-[#9a6b31] mb-3">
              Shop by use
            </p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {useCards.map((card) => (
            <Link
              key={card.title}
              to={`/room-sprays?use=${encodeURIComponent(card.title.toLowerCase())}`}
              className="group relative aspect-[4/3] overflow-hidden bg-[#2a2119]"
            >
              <img
                src={card.image}
                alt={card.title}
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 md:p-5 text-white">
                ...
                <div className="mt-2 flex items-center justify-between text-xs tracking-wide">
                  <span>SHOP NOW</span><span className="text-xl">↗</span>
                </div>
              </div>
            </Link>
          ))}
