...
(() => {
  ...
  function cleanSprayCards(root=document){
    root.querySelectorAll('#collection .product-card').forEach(card=>{
      card.querySelectorAll('.type').forEach(el=>el.remove());
    });
  }
  function applyNewSprayImages(root=document){
    root.querySelectorAll('img').forEach(img=>{
      ...
      if(key&&img.getAttribute('src')!==replacements[key]){img.src=replacements[key];img.removeAttribute('srcset');}
    });
  }
  function addNewSprays(){
    ...
    newSprays.forEach(product=>{
      ...
      const article=document.createElement('article');article.className='product-card';article.dataset.newSpray=product.id;
      article.innerHTML=`<div class="product-image portrait-product"><a class="product-photo-link" href="/room-sprays/#collection" aria-label="${arabic?'عرض':'View'} ${name}"><img src="${product.image}" alt="${name}" loading="lazy" width="1024" height="1024"></a><a class="product-quick" href="/room-sprays/#collection">${arabic?'اكتشف العطر':'Discover Fragrance'}</a></div><h3><a href="/room-sprays/#collection">${name}</a></h3><p class="price">QAR 120</p><button class="card-add new-spray-order" type="button">${arabic?'أضف إلى الحقيبة':'Add to Bag'}</button>`;
      article.querySelector('.new-spray-order').addEventListener('click',()=>{const message=arabic?`أرغب في طلب ${name} - QAR 120`:`I would like to order ${product.name} - QAR 120`;window.open(`https://wa.me/97433901711?text=${encodeURIComponent(message)}`,'_blank','noopener');});
  ...
  function applyRoomSprayLayout(){
    ...
    if(cardsGrid){cardsGrid.classList.add('spray-use-cards');const cards=Array.from(cardsGrid.querySelectorAll('.category-card')),images=['/assets/use-room-mist.png','/assets/use-car-mist.png','/assets/use-clothing-mist.png'];cards.slice(0,3).forEach((card,index)=>{const img=card.querySelector('img');if(img&&img.getAttribute('src')!==images[index]){img.src=images[index];img.removeAttribute('srcset');}if(img)img.style.cssText='position:absolute;inset:0;width:100%;height:100%;margin:0;object-fit:cover;object-position:center;max-width:none;transform:none;mask-image:none;';const overlay=card.querySelector('div');if(overlay)overlay.style.display='none';card.style.cssText='position:relative;display:block;width:100%;height:auto;aspect-ratio:307/204;overflow:hidden;background:#1c1712;';});cards.slice(3).forEach(card=>card.remove());cardsGrid.style.cssText='display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;width:100%;justify-content:stretch;align-items:start;';}
